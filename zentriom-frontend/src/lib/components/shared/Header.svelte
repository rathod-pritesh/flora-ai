<script>
	...
	import { Menu, Settings, LogOut } from 'lucide-svelte';
...
<header
	class="flex h-16 items-center justify-between border-b border-border bg-card text-foreground px-4 lg:px-6"
>
	<div class="flex items-center gap-4">
		<button
			onclick={() => (appState.mobileSidebarOpen = true)}
			class="flex size-9 items-center justify-center rounded-md border border-border text-foreground lg:hidden hover:bg-muted select-none outline-none cursor-pointer"
