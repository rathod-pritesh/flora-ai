...
<div class="grid grid-cols-1 lg:grid-cols-2 gap-6 items-start text-foreground">
	...
	<Card class="bg-card border-border shadow-xs">
		<CardHeader class="border-b border-border">
			...
			<CardDescription class="text-xs text-muted-foreground font-sans">Paste or write your drafts for instant correction</CardDescription>
		</CardHeader>
		<CardContent class="p-6 space-y-4">
			...
			<div class="flex items-center justify-between">
				<span class="text-[10px] text-muted-foreground font-sans">{sourceText.length} characters</span>
			...
			<Card class="bg-card border-border shadow-xs">
				<CardHeader class="border-b border-border flex flex-row items-center justify-between pb-4">
					<div>
						...
						<CardDescription class="text-xs text-muted-foreground font-sans">Review proposed enhancements</CardDescription>
					</div>
					<Button onclick={applyAll} variant="outline" class="border-border hover:bg-muted h-8 text-xs font-sans bg-card text-foreground">
						Apply All
					</Button>
				</CardHeader>
				<CardContent class="p-6 space-y-4">
					{#each suggestions as sug}
						<div class="p-3 border border-border rounded-lg space-y-2 bg-muted/30">
							<div class="flex items-center justify-between">
								<Badge variant="secondary" class="text-[10px] capitalize {sug.type === 'grammar' ? 'bg-red-50 text-red-700 dark:bg-red-500/10 dark:text-red-400' : 'bg-amber-50 text-amber-700 dark:bg-amber-500/10 dark:text-amber-400'}">
									{sug.type}
								</Badge>
							...
							<div class="text-xs leading-relaxed text-foreground">
								<p class="font-sans"><span class="line-through text-red-400">{sug.original}</span> <span class="font-semibold text-emerald-600 dark:text-emerald-450">{sug.replacement}</span></p>
		...
		<Card class="bg-card border-border shadow-xs">
			<CardHeader class="border-b border-border flex flex-row items-center justify-between pb-4">
				<div>
					...
					<CardDescription class="text-xs text-muted-foreground font-sans">Polished draft ready for submission</CardDescription>
				</div>
				<Button onclick={handleCopy} variant="outline" class="border-border hover:bg-muted h-8 text-xs font-sans bg-card text-foreground">
					{#if copyStatus}
			...
			<CardContent class="p-6">
				<p class="text-xs leading-relaxed text-foreground font-sans min-h-[80px] whitespace-pre-wrap">
					{correctedText || sourceText}
				</p>
			</CardContent>
		</Card>
