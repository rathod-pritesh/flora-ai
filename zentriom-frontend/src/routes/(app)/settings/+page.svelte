...
<div class="max-w-4xl mx-auto space-y-6 text-foreground">
	...
	<Card class="bg-card border-border shadow-xs">
		<CardHeader class="border-b border-border">
			<CardTitle class="text-base font-bold text-foreground font-sans"
				>Account Information</CardTitle
			>
			<CardDescription class="text-xs text-muted-foreground font-sans"
				>Your Zentriom profile and login credentials</CardDescription
			>
		</CardHeader>
		<CardContent class="p-6">
			{#if !authStore.user}
				<div class="flex items-center gap-2 text-muted-foreground text-xs font-sans py-2">
					<div
						class="size-4 border-2 border-[#A16207] border-t-transparent rounded-full animate-spin"
					></div>
			...
			{:else}
				<div class="grid grid-cols-1 md:grid-cols-3 gap-6">
					<div>
						<span
							class="block text-[10px] font-bold text-muted-foreground uppercase tracking-wider font-sans"
							>Full Name</span
						>
						<p class="text-xs font-bold text-foreground font-sans mt-1">
					...
					<div>
						<span
							...
							>Email Address</span
						>
						<p class="text-xs font-semibold text-muted-foreground font-sans mt-1">
					...
					<div>
						...
						<div class="mt-1">
							<Badge
								class="bg-emerald-55 text-emerald-700 border-emerald-200 text-[9px] uppercase tracking-wider font-sans"
							>
								Active
							</Badge>
						</div>
	...
	<Card class="bg-card border-border shadow-xs">
		<CardHeader class="border-b border-border">
			...
			<CardDescription class="text-xs text-muted-foreground font-sans"
				>Choose how Zentriom interface looks on your browser</CardDescription
			>
		</CardHeader>
		<CardContent class="p-6">
			<div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
				...
				>
					...
					<span class="text-[10px] text-muted-foreground font-sans"
						>Syncs with operating system</span
					>
				</button>
			</div>
		</CardContent>
	...
	<Card class="bg-card border-border shadow-xs">
		<CardHeader class="border-b border-border">
			...
			<CardDescription class="text-xs text-muted-foreground font-sans"
				>Configuration settings for AI model capabilities</CardDescription
			>
		</CardHeader>
		<CardContent class="p-6">
			<div class="space-y-4">
				<div
					class="p-4 rounded-xl border border-[#A16207]/30 bg-[#A16207]/5 text-left flex items-start gap-4"
				>
					<div
						class="mt-0.5 shrink-0 flex items-center justify-center size-5 rounded-full border border-[#A16207]/30 bg-card"
					>
					...
					<div class="flex-1 min-w-0">
						<div class="flex flex-wrap items-center justify-between gap-2">
							<h4 class="text-xs font-bold text-foreground font-sans flex items-center gap-2">
								...
								<span
									class="bg-[#A16207]/10 text-[#A16207] text-[8px] font-bold uppercase tracking-wider px-1.5 py-0.5 rounded-full"
									>Active</span
								>
							</h4>
							<span
								class="text-[9px] text-emerald-600 font-semibold uppercase tracking-wider flex items-center gap-1"
							>
								<span class="size-1.5 rounded-full bg-emerald-500"></span> Connected
							</span>
						</div>
						<p class="text-[10px] text-muted-foreground font-sans mt-1">
							Locally optimized models providing secure text completions, explanation patterns, and
							programming outputs.
						</p>
					</div>
				</div>
	...
	<Card class="bg-card border-border shadow-xs">
		<CardHeader class="border-b border-border">
			<CardTitle class="text-base font-bold text-foreground font-sans"
				>Workspace Management</CardTitle
			>
			<CardDescription class="text-xs text-muted-foreground font-sans"
				>Manage your workspace history and account deletion settings</CardDescription
			>
		</CardHeader>
		<CardContent class="p-6">
			<div class="w-full">
				<div
					class="relative p-4 rounded-xl border border-border bg-muted/50 opacity-70 select-none"
				>
					<div
						class="absolute top-3 right-3 bg-muted text-muted-foreground border border-border text-[8px] font-bold uppercase tracking-wider px-2 py-0.5 rounded-full font-sans"
					>
						UNDER DEVELOPMENT
					</div>
					...
					<p class="text-[10px] text-muted-foreground font-sans mt-1">
						Permanently closes your account and removes all related user files.
					</p>
				</div>
			</div>
		</CardContent>
